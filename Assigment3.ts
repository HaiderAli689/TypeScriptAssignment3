...
function removeNegativeNumbers(numbers: number[]){
...
const temptofht:number[]= celsius.map((fht) => (fht *9/5) +32)
...
var numbers1:number[] = [3, 6, 9, 12, 15, 18]
const doubleOddValues:number[] = numbers1.filter((num) => num % 2 != 0)
.map((odd) => odd *2 );
